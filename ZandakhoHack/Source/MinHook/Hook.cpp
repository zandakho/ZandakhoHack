...
#if HOOK_INCLUDE_D3D9
# include <d3d9.h>
#endif

#if HOOK_INCLUDE_D3D10
# include <dxgi.h>
# include <d3d10_1.h>
# include <d3d10.h>
#endif

#if HOOK_INCLUDE_D3D11
# include <dxgi.h>
# include <d3d11.h>
#endif

#if HOOK_INCLUDE_D3D12
# include <dxgi.h>
# include <d3d12.h>
#endif

#if HOOK_INCLUDE_OPENGL
# include <gl/GL.h>
#endif

#if HOOK_INCLUDE_VULKAN
...
#ifdef _UNICODE
# define HOOK_TEXT(text) L##text
#else
# define HOOK_TEXT(text) text
#endif

#define HOOK_ARRAY_SIZE(arr) ((size_t)(sizeof(arr)/sizeof(arr[0])))

static Hook::RenderType::Enum g_renderType = Hook::RenderType::None;
static uint150_t* g_methodsTable = NULL;

Hook::Status::Enum Hook::Init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.lpszClassName = HOOK_TEXT("ZHACK");
			windowClass.hIconSm = NULL;

			::RegisterClassEx(&windowClass);

			HWND window = ::CreateWindow(windowClass.lpszClassName, HOOK_TEXT("DirectX Window"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);

			if (_renderType == RenderType::D3D9)
			{
#if HOOK_INCLUDE_D3D9
				HMODULE libD3D9;
				if ((libD3D9 = ::GetModuleHandle(HOOK_TEXT("d3d9.dll"))) == NULL)
				{
			...
			else if (_renderType == RenderType::D3D10)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D10;
				if ((libDXGI = ::GetModuleHandle(HOOK_TEXT("dxgi.dll"))) == NULL || (libD3D10 = ::GetModuleHandle(HOOK_TEXT("d3d10.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
...
#if HOOK_INCLUDE_D3D11
				HMODULE libD3D11;
				if ((libD3D11 = ::GetModuleHandle(HOOK_TEXT("d3d11.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
			...
			else if (_renderType == RenderType::D3D12)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D12;
				if ((libDXGI = ::GetModuleHandle(HOOK_TEXT("dxgi.dll"))) == NULL || (libD3D12 = ::GetModuleHandle(HOOK_TEXT("d3d12.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
		...
		else if (_renderType == RenderType::OpenGL)
		{
#if HOOK_INCLUDE_OPENGL
			HMODULE libOpenGL32;
			if ((libOpenGL32 = ::GetModuleHandle(HOOK_TEXT("opengl32.dll"))) == NULL)
			{
				return Status::ModuleNotFoundError;
			}
			...
			size_t size = HOOK_ARRAY_SIZE(methodsNames);
...
#if HOOK_INCLUDE_VULKAN
			HMODULE libVulkan;
			if ((libVulkan = GetModuleHandle(HOOK_TEXT("vulcan-1.dll"))) == NULL)
			{
				return Status::ModuleNotFoundError;
			}
		...
		else if (_renderType == RenderType::Auto)
		{
			...
			if (::GetModuleHandle(HOOK_TEXT("d3d9.dll")) != NULL)
			{
				type = RenderType::D3D9;
			}
			else if (::GetModuleHandle(HOOK_TEXT("d3d10.dll")) != NULL)
			{
				type = RenderType::D3D10;
			}
			else if (::GetModuleHandle(HOOK_TEXT("d3d11.dll")) != NULL)
			{
				type = RenderType::D3D11;
			}
			else if (::GetModuleHandle(HOOK_TEXT("d3d12.dll")) != NULL)
			{
				type = RenderType::D3D12;
			}
			else if (::GetModuleHandle(HOOK_TEXT("opengl32.dll")) != NULL)
			{
				type = RenderType::OpenGL;
			}
			else if (::GetModuleHandle(HOOK_TEXT("vulcan-1.dll")) != NULL)
			{
...
Hook::Status::Enum Hook::Bind(uint16_t _index, void** _original, void* _function)
{
...
void Hook::Unbind(uint16_t _index)
{
	assert(_index >= 0);
...
Hook::RenderType::Enum Hook::GetRenderType()
{
	return g_renderType;
}

uint150_t* Hook::GetMethodsTable()
{
